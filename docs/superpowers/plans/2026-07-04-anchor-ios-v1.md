...
- **Task 3.2 — Day timeline:** `DayViewModel` DONE + green (run d1f72a0): load, toggle-done (mint win / never remove on undo), lossless mode switch, shift-my-day, convert-to-rest, buffer advice; 8 VM tests. **Agenda visualisation** + mode toggle + shift action wired into the Day tab. **Pending next Day increment:** Ribbon and Focus visualisations, `BlockDetailSheet`/`BlockEditorSheet` (add/edit blocks + steps), `TemplatePickerSheet` + save-as-template.
